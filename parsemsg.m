function [str,msgType,msgID] = parsemsg(msg)
% PARSE_MSG Utility function to parse some messages from MRM API.

header = dec2hex(typecast([msg(2) msg(1)],'uint16'));
if strcmp(header,'A5A5')
  msg = msg(5:end);
end

msgType = dec2hex(typecast([msg(2) msg(1)],'uint16'));
...
switch msgType
  ...
  otherwise
    error('Invalid msgType: %s',msgType);
    %str = [];
