function MrmRadarDemo(varargin)
...
Rstart_m = 0;  % This is the distance from the tx antenna at which to start the scan window
Rstop_m = 5;  % This is the distance to end the scan window
...
cfgStruct.antennaMode = uint8(2);  % 2: Tx on B, Rx on A.  3: Tx on A, Rx on B
%cfgStruct.transmitGain = uint8(0);  % for P400 (has 13dB amp. Set to 0 for FCC power.)
cfgStruct.transmitGain = uint8(60); % for P410 (no amp - set to max gain.)
cfgStruct.pulseIntegrationIndex = uint16(13);
...
while goEnable
  ...
  detectMode = 'SIMPLE2';
  minThreshold = 0.2;
  [detectList,threshold] = detect(Rbin_m,scan_env,detectMode,minThreshold);
  ...
  if i > memDepth
    ...
    switch guiType
      ...
      case WATERFALL_PLOT
        ...
        plotwaterfall(gca, Rbin_m, scanDisp_dB, detectList, slowTimeDepth,[-35 -5]);
      case WEDGE_PLOT
